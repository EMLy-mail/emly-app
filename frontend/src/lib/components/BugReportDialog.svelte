<script lang="ts">
    ...
    $effect(() => {
        if (dev && $bugReportDialogOpen && !userName) {
            ...
            bugDescription =
                "This is a test bug report submitted from development mode.";
        }
    });
    ...
    >
        {#if isSuccess}
            ...
            <Dialog.Header>
                ...
                <Dialog.Description>
                    {#if uploadedToServer}
                        {m.bugreport_uploaded_success({
                            reportId: String(serverReportId),
                        })}
                    {:else}
            ...
            <div class="grid gap-4 py-4">
                {#if uploadError}
                    ...
                    >
                        <AlertTriangle
                            class="h-4 w-4 text-yellow-500 mt-0.5 shrink-0"
                        />
